...
import com.pujaad.coopmart.databinding.FragmentInventoryCategoryBinding
...
import com.pujaad.coopmart.view.main_menu.ui.inventory_category.transaction.InventoryCategoryTransactionFragment
import com.pujaad.coopmart.viewmodel.InventoryViewModel

class InventoryCategoryFragment : BaseFragment(R.layout.fragment_inventory_category) {
    ...
    private lateinit var binding: FragmentInventoryCategoryBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentInventoryCategoryBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        setFragmentResultListener(InventoryCategoryTransactionFragment.REQUEST_KEY) { key, bundle ->
            val operationStatus = bundle.getBoolean(Constant.OPERATION_STATUS, false)
            if (operationStatus) {
                inventoryCategoryAdapter?.submitList(arrayListOf<ProductCategory>())
                val query = binding.etSearchInventoryCategory.text.toString()
                viewModel.lookupInventoryCategory(query = query)
            }
        }
        ...
        inventoryCategoryAdapter?.listener = object : AdapterListener<ProductCategory> {
            override fun onclick(item: ProductCategory) {
                findNavController().navigate(InventoryCategoryFragmentDirections.categoryToEditCategory(category = item))
            }
        ...
        binding.fabAdd.setOnClickListener {
            findNavController().navigate(InventoryCategoryFragmentDirections.categoryToAddCategory())
        }
    }
